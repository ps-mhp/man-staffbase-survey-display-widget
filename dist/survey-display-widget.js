...
// UNUSED EXPORTS: INSTALLATION_ID_ATTRIBUTE, PLUGIN_URL_ATTRIBUTE, readPluginUrl, stopSurveyPickerInjector
...
;// ./src/widgets/survey-display-widget/src/survey-attributes.ts
/*!
...
const SURVEY_ELEMENT = "survey-plugin-employee-block";

/** Where the survey plugin lives unless the author says otherwise. */
const DEFAULT_PLUGIN_URL = "https://pluginsurveys-de1.staffbase.com";
 ...
 */
function readInstallationId(raw) {
...
function documentLocale() {
  ...
  const candidates = [document.documentElement.getAttribute("lang"), meta?.getAttribute("content"), navigator.language];
 ...
 */
function buildSurveyAttributes(input) {
  const {
    installationId,
    pluginUrl,
    branch,
...
;// ./src/widgets/survey-display-widget/src/configuration-schema.ts
/*!
 * Copyright 2026, MHP Management und IT-Beratung GmbH and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
const uiSchema = {
  "installation-id": {
    "ui:help": "ID der Umfragen-Installation — die 24-stellige Zeichenfolge am Ende der Umfragen-URL. " + "Die vollständige URL kann ebenfalls eingefügt werden."
  },
  "plugin-url": {
    "ui:help": "Nur ändern, wenn die App in einer anderen Region liegt. Voreingestellt ist " + `${DEFAULT_PLUGIN_URL}.`
  }
};
;// ./src/shared/entity-picker/entity-catalog.ts
/*!
 * Copyright 2026, Staffbase SE and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
/** An entity as a configuration dialog picker needs to show it. */

/**
 * How a widget's picker gets its catalog.
 *
 * `fetchList` is the only part `fetchEntityCatalog` protects the caller from:
 * a rejection or thrown error there becomes an empty list. `toOption` may
 * itself perform I/O (e.g. a per-id detail lookup) and, if it does, must
 * catch its own errors and return a fallback — a rejection out of `toOption`
 * is not caught here and fails the whole `Promise.all`.
 */

/**
 * The entities this author may put into a widget.
 *
 * Never rejects because of `fetchList`. The dialog falls back to the plain id
 * field when the list stays empty, so a failed request costs a convenience —
 * while an exception here would take the whole configuration dialog with it.
 */
async function fetchEntityCatalog(source) {
  let entries;
  try {
    entries = await source.fetchList();
  } catch {
    return [];
  }
  const options = await Promise.all(entries.map(entry => source.toOption(entry)));
  return options.filter(option => option !== null);
}
;// ./src/shared/config-field-injector.tsx
/*!
 * Copyright 2026, Staffbase SE and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
/**
 * Custom user interface inside the widget configuration dialog.
 *
 * The dialog is built by the host from a widget's JSON schema, and the SDK
 * offers no hook to take part in it: a schema describes fields, not behaviour,
 * and it is fixed at `defineBlock` time, before anything is known that a field
 * might need to look up. Anything richer than a text box therefore has to find
 * the rendered field in the DOM and mount next to it.
 *
 * That makes this module a bet on the host's internals — the id RJSF derives
 * from a schema key, and the way React notices a programmatic value change.
 * Both are outside our control and can break with a Staffbase update. Keeping
 * the bet in one place is the point of this file: it is then corrected once,
 * not once per widget.
 */



/** A form field as RJSF renders it. */

/**
 * The selector for the element RJSF renders for a schema property.
 *
 * RJSF derives the id from the property path, so `root_` plus the key. The key
 * is escaped because it is author-controlled and may contain characters that
 * mean something in CSS.
 */
function configFieldSelector(fieldKey) {
  const escape = window.CSS?.escape ?? (value => value);
  return `#${escape(`root_${fieldKey}`)}`;
}

/**
 * Writes `value` into `field` the way a user would.
 *
 * A plain assignment does not reach React. It replaces the `value` setter on
 * the element instance, so the framework never learns of the change and the
 * dialog would save the value that was there before. Going through the
 * prototype's setter and then dispatching a bubbling `input` event is what
 * React's synthetic event system listens for.
 */
...
/**
 * Watches for a configuration field and mounts custom user interface in front
 * of it.
 *
 * Safe to call at module load, before any dialog exists, and safe where none
 * ever appears: on a published page the field is never rendered, the observer
 * never matches, and nothing beyond the observer itself is paid for.
 *
 * Mounting is idempotent per field, and anything mounted for a field that has
 * since left the DOM is unmounted, so an opened and closed dialog leaks no
 * React roots.
 *
 * @returns a function that stops watching and unmounts what was injected.
 */
function startConfigFieldInjector(options) {
  const {
    fieldKey,
    render,
    hideField = false,
  ...
  const selector = configFieldSelector(fieldKey);
  const mounted = new Map();
  const injectInto = field => {
    if (mounted.has(field)) return;
    if (hideField) field.style.display = "none";
    const mountPoint = document.createElement("div");
    mountPoint.className = "config-field-mount";
    field.insertAdjacentElement("beforebegin", mountPoint);
    const mountRoot = client.createRoot(mountPoint);
    mountRoot.render(render(field));
    mounted.set(field, {
      root: mountRoot,
      mountPoint
    });
...
;// ./src/shared/entity-picker/entity-picker.tsx
/*!
 * Copyright 2026, Staffbase SE and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
/**
 * Picks an entity from the list, or steps aside so the id can be typed.
 *
 * Stepping aside is not a fallback for errors alone. An id that the list does
 * not contain — one outside this author's permissions, or one saved before the
 * list existed — must not be represented by an empty dropdown: the value would
 * look unset, and the next save would quietly drop it.
 */
function EntityPicker({
  ...
  if (stepAside) {
    ...
    return /*#__PURE__*/react.createElement(react.Fragment, null, options.length === 0 && /*#__PURE__*/react.createElement("p", {
      style: noticeStyle
    }, labels.unavailableNotice), extra);
  }
  return /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement("select", {
    ...
    value: ""
  }, labels.placeholder), options.map(option => /*#__PURE__*/react.createElement("option", {
    key: option.id,
    value: option.id,
    disabled: option.disabled
  }, option.title)), /*#__PURE__*/react.createElement("option", {
    value: MANUAL_OPTION_VALUE
  }, labels.manualOption)), extra);
}
;// ./src/shared/entity-picker/entity-picker-injector.tsx
/*!
 * Copyright 2026, Staffbase SE and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
/** Lets `InjectedPicker` subscribe to "something worth re-fetching for changed". */
...
function InjectedPicker({
  ...
  // Mirrors `input.value`. The field is owned by RJSF and mutated straight in
  // the DOM (by this picker's own `onChange`, or by anything else that calls
  // `setNativeFieldValue` on it, e.g. a re-fetch elsewhere) — never through a
  // React prop this component would naturally re-render for. Without this
  // state, the rendered `<select>` keeps showing whatever `input.value` was
  // at the last unrelated re-render, which is indistinguishable from "the
  // selection didn't take" to whoever is looking at the dialog.
  const [value, setValue] = (0,react.useState)(input.value);
  (0,react.useEffect)(() => refreshBus.subscribe(() => setRefreshToken(token => token + 1)), [refreshBus]);
  (0,react.useEffect)(() => {
    let current = true;
    // A refresh past the first (`refreshToken > 0`) means a *watched* field
    // changed — the id this field currently holds almost certainly belonged
    // to whatever the watched field was set to before, e.g. an episode id
    // from the podcast the author just switched away from. Left in place, it
    // survives into the new catalog as an id the list does not contain, and
    // the picker steps aside to raw editing — showing a stray, foreign id
    // instead of the fresh list. A first load carries no such assumption: a
    // saved id the catalog does not (yet) contain there is exactly the case
    // manual editing exists for, so it is left untouched.
    const isWatchedRefresh = refreshToken > 0;
    setLoading(true);
    fetchOptions().then(loaded => {
      if (!current) return;
      setOptions(loaded);
      setLoading(false);
      if (isWatchedRefresh && input.value !== "" && !loaded.some(option => option.id === input.value)) {
  ...
  }, [fetchOptions, refreshToken, input]);

  // Keeps `value` in sync with the field regardless of who changed it: this
  // picker's own `onChange` dispatches the same `input` event `setNativeFieldValue`
  // always fires, so one listener covers both self-driven and outside changes.
  (0,react.useEffect)(() => {
  ...
  const handleManual = (0,react.useCallback)(() => {
    setManual(true);
    input.focus();
  }, [input]);
  if (loading) return null;
...
/**
 * Watches for the widget's configuration dialog and puts a list in front of
 * one of its id fields.
 *
 ...
 */
function startEntityPickerInjector(options) {
  const {
    fieldKey,
    fetchOptions,
    labels,
  ...
  const watchedSelectors = watchFields.map(watchedKey => configFieldSelector(watchedKey));
  const handleWatchedInput = event => {
    const target = event.target;
    if (!(target instanceof Element)) return;
    if (watchedSelectors.some(selector => target.matches(selector))) {
      listeners.forEach(listener => listener());
    }
  };

  // Capture phase, so a watched field's own `input` event is seen here
  // regardless of whether something else along the bubbling path stops it.
  const eventTarget = root;
  if (watchedSelectors.length > 0) {
    eventTarget.addEventListener("input", handleWatchedInput, true);
  }
  const stopInjector = startConfigFieldInjector({
    fieldKey,
    root,
    render: input => /*#__PURE__*/react.createElement(InjectedPicker, {
      input: input,
  ...
  return () => {
    stopInjector();
    if (watchedSelectors.length > 0) {
      eventTarget.removeEventListener("input", handleWatchedInput, true);
    }
    listeners.clear();
  };
}
;// ./src/shared/entity-picker/localized-title.ts
/*!
 * Copyright 2026, Staffbase SE and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 ...
 *
 * Falls back to English and then to whatever exists: an entity translated
 * into neither is still one an author may want to pick, and a blank row would
 * be worse than a foreign one.
 */
function pickLocalizedTitle(localization) {
...
;// ./src/widgets/survey-display-widget/src/survey-catalog.ts
/*!
 * Copyright 2026, MHP Management und IT-Beratung GmbH and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
const CATALOG_ENDPOINT = "/api/plugins/surveys/installations/search";
...
const surveyCatalogSource = {
  ...
  toOption(entry) {
    ...
    return {
      id,
      title: pickLocalizedTitle(entry.data?.config?.localization) ?? id
    };
  }
};
;// ./src/widgets/survey-display-widget/src/plugin-loader.ts
/*!
 * Copyright 2026, MHP Management und IT-Beratung GmbH and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
function resetLoaderForTests() {
  loads.clear();
}

/** The plugin's employee bundle under a given plugin host. */
function bundleUrl(pluginUrl) {
 ...
 */
function loadSurveyPlugin(pluginUrl) {
...
;// ./src/widgets/survey-display-widget/src/survey-view.tsx
/*!
 * Copyright 2026, MHP Management und IT-Beratung GmbH and contributors.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
const MISSING_ID = "Keine Installations-ID konfiguriert. Bitte die ID der Umfrage in den Widget-Einstellungen eintragen.";
/**
...
function SurveyView({
  ...
  const [state, setState] = react.useState(installationId === null ? {
    status: "error",
    message: MISSING_ID
  } : {
    status: "loading"
  ...
  react.useEffect(() => {
    ...
    Promise.all([loadSurveyPlugin(pluginUrl), fetchUserLocale()]).then(([, locale]) => {
      ...
      setState({
        status: "error",
        message: `Umfrage konnte nicht geladen werden: ${reason}`
      });
    });
    return () => {
      current = false;
  ...
  return /*#__PURE__*/react.createElement("div", {
    className: "survey-display",
    "data-testid": "survey-display"
  }, state.status === "loading" && /*#__PURE__*/react.createElement("p", {
    className: "survey-display__status"
  }, "Umfrage wird geladen \u2026"), state.status === "error" && /*#__PURE__*/react.createElement("p", {
    className: "survey-display__error",
    role: "alert"
  }, state.message), /*#__PURE__*/react.createElement("div", {
    ref: host
  }));
}
;// ./src/widgets/survey-display-widget/resources/survey-display-widget.svg
/* harmony default export */ var survey_display_widget = ("data:image/svg+xml;base64,");
;// ./src/widgets/survey-display-widget/package.json
var package_namespaceObject = /*#__PURE__*/JSON.parse('{"rE":"0.1.0-rc.2","cy":""}');
;// ./src/widgets/survey-display-widget/src/index.tsx
 ...
 * `widgets.json`.
 */
const INSTALLATION_ID_ATTRIBUTE = "installation-id";
const PLUGIN_URL_ATTRIBUTE = "plugin-url";

/** Attributes handled by the widget; mirrored in the configuration schema. */
const widgetAttributes = [INSTALLATION_ID_ATTRIBUTE, PLUGIN_URL_ATTRIBUTE];
...
const SURVEY_PICKER_LABELS = {
  ...
  unavailableNotice: "Die Liste der Umfragen konnte nicht geladen werden. Bitte die Installations-ID eintragen."
};
const stopSurveyPickerInjector = startEntityPickerInjector({
...
const factory = (BaseBlockClass, widgetApi) => {
  return class SurveyDisplayWidgetBlock extends BaseBlockClass {
    constructor(...args) {
      super(...args);
    ...
    renderBlock(container) {
      ...
      this._root.render(/*#__PURE__*/react.createElement(SurveyView, {
...
/******/ 	var __webpack_exports__ = __webpack_require__(2509);
/******/ 	
/******/ })()
;